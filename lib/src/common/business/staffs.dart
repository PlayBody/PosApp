...
class ClStaff {
  Future<StaffModel?> login(context, staffMail, password) async {
    ...
    try {
      if (results['is_load'] != null) {
        return StaffModel.fromJson(results['data']);
      }
    } catch (e) {
      return null;
    }
  ...
  Future<StaffModel> loadStaffInfo(BuildContext context, staffId) async {
    ...
    print(apiStaffDetail);
    print(staffId);
    await Webservice().loadHttp(
        context, apiStaffDetail, {'staff_id': staffId}).then((v) => {results = v});
    if (results['is_load']) {
      staffInfo = StaffModel.fromJson(results['data']);
    }
