...
class ClCommon {
  ...
  */
  ...
  Future<List<RankModel>> loadRanks(context, companyId) async {
    Map<dynamic, dynamic> results = {};
    await Webservice().loadHttp(context, apiLoadRanks, {
  ...
  Future<int> loadUserStampCount(context, userId) async {

    Map<dynamic, dynamic> stampResults = {};
    await Webservice().loadHttp(context, apiLoadUserStampCntUrl, {
      'user_id': userId
    }).then((value) => stampResults = value);

    int cnt = 0;
    if (stampResults['is_load']) {
      cnt = stampResults['count'];
    }
