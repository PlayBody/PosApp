...
class PosPrinters {
  ...
  Future<void> runPrintTicket(printData) async {
    ...
    printData['menus'].forEach((e) {
      ...
      _socket.add(generator.row([
        ...
        PosColumn(
            text: Funcs()
                .currencyFormat((menuPrice * int.parse(e.quantity)).toString()),
            width: 2,
            styles: const PosStyles(align: PosAlign.right)),
      ]));
  ...
  Future<void> runReceiptPrint(context, printData, organId) async {
    ...
    if (organId != '') {
      ...
      if (organ.isServiceTax != null && organ.isServiceTax == "1") {
        isServiceTax = true;
    ...
    for (var item in printData['menus']) {
      double taxRate =
          (1 + (item.menuTax == '' ? 0 : (int.parse(item.menuTax) / 100)))
              .toDouble();
      String am = (double.parse(item.menuPrice).toInt() *
              taxRate *
      ...
      cnt++;
      sum += (double.parse(item.menuPrice).toInt() *
              taxRate *
              int.parse(item.quantity))
          .toInt();
    }
